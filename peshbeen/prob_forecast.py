...
class ml_prob_forecasts():
    ...
    def sample_predictions(self, df, samples=1000, future_exog=None):
        ...
        if future_exog is not None:
            ...
            y_forecast = np.array(self.model.forecast(self.H))
        self.sampled_resids = np.column_stack([[gaussian_kde(self.resid[i]).resample(size=1)[0][0]
        ...
        abs_sampled_resids = np.abs(self.sampled_resids)
        quantiles = self.calculate_quantile(abs_sampled_resids.T)
        if isinstance(self.delta, float):
