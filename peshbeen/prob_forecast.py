...
class ml_prob_forecasts():
    ...
    def bootstrap(self, df, samples=1000, future_exog=None):
        ...
        self.bootstrap_forecasts_df = pd.DataFrame(self.bootstrap_forecasts.T, columns=[f'h_{i+1}' for i in range(self.H)])
        ...
        lower_bound = np.quantile(self.bootstrap_forecasts.T, low, method="lower", axis=0)
        upper_bound = np.quantile(self.bootstrap_forecasts.T, high, method="higher", axis=0)
...
class arima_prob_forecasts():
    ...
    def calibrate(self, df, target_col):
